...
If you switch a game please detect cart before starting the game

The Z menu also controls the scale of the emulation on your display. The settings.ini file controls the default scale as well as the dev menu option which has some more features and is less “plug and play”. Borders placed in the border folder are examples that you can use as templates. The naming convention is border_XXXX with the x’s being the game code on your cart. For example, English fire red is BPRE. Save borders as a .bmp file. I used Aseprite and it worked well.
